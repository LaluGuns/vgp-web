...
test('Meta OAuth trusts only account and grants returned by the token exchange', async () => {
    const calls = [];
    const client = new meta.MetaProviderClient(
        {
            appId: 'app-id',
    ...
    assert.equal(tokens.accountId, 'ig-account-1');
    assert.equal(tokens.accountLabel, null);
...
test('Meta OAuth fails closed when the token exchange omits grants', async () => {
    const client = new meta.MetaProviderClient(
        {
            appId: 'app-id',
            appSecret: 'app-secret',
    ...
    await assert.rejects(
        ...
        (error) =>
            ...
            && error.providerCode === 'INCOMPLETE_TOKEN_GRANTS'
    );
});
