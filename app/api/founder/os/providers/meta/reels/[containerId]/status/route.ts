...
export async function GET(
    ...
    const containerId = (await params).containerId;
    if (!SAFE_PROVIDER_ID.test(containerId)) {
        return founderOsJson(
            { success: false, error: 'Invalid container ID.', requestId },
            { status: 400 }
        );
    }
    try {
        const summaries = await listProviderConnectionSummaries('meta');
        const summary = summaries.find((item) => item.status === 'connected');
        if (!summary) {
            return founderOsJson(
                { success: false, error: 'Meta is not connected.', requestId },
                { status: 409 }
            );
        }
        const decrypted = await loadProviderCredentialsForServer(summary.id);
        const status = await getMetaClient().getReelContainerStatus(
