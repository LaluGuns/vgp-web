...
Pattern interrupt: empty pre-beat space pulling the body forward appears before the full headline, with the pre-beat vacuum marked in cyan and the problem marked in red.
Open loop: The viewer waits to see which decision makes rhythmic silence clearer in the session.
Retention payoff: The payoff is a practical test: Cut the last hat before the downbeat.
Visual identity: empty pre-beat space pulling the body forward; motif: pre-beat vacuum; camera: top-down rhythm grid with human movement cues around it; motion: timing dots drift by a few pixels while the beat grid stays fixed.
Motion plan: 0-3s empty pre-beat space pulling the body forward appears before the full headline, with the pre-beat vacuum marked in cyan and the problem marked in red; 3-9s show the symptom; 9-18s map temporal masking and contrast; 18-24s run the move, Remove a tiny pre-hit sound that softens the attack; 24-30s prove it with Cut the last hat before the downbeat and close on The next hit feels bigger.
