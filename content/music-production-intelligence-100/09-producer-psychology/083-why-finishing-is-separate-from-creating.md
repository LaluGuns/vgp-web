# 083 - Finishing uses a different brain state
...
Pattern interrupt: create mode and finish mode split across one desk appears before the full headline, with the mode switch marked in cyan and the problem marked in red.

Open loop: The viewer waits to see which decision makes finishing music clearer in the session.

Retention payoff: The payoff is a practical test: Write three fixes max before the next bounce.

Visual identity: create mode and finish mode split across one desk; motif: mode switch; camera: producer desk scene with task timer, DAW, and physical notes; motion: messy options fade down until one decision remains active.

Motion plan: 0-3s create mode and finish mode split across one desk appears before the full headline, with the mode switch marked in cyan and the problem marked in red; 3-9s show the symptom; 9-18s map workflow psychology; 18-24s run the move, Define the finish line before opening new tracks; 24-30s prove it with Write three fixes max before the next bounce and close on The project leaves the loop.
...
Slide 1 thumbnail read: Finishing uses different brain
...
Science anchor: workflow psychology

Art and taste anchor: A finished song is a decision trail.
