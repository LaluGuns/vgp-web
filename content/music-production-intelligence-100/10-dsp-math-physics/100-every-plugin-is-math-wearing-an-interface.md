# 100 - Every plugin is math with knobs
...
Pattern interrupt: plugin interface peeled back to reveal math knobs appears before the full headline, with the math knobs marked in cyan and the problem marked in red.

Open loop: The viewer waits to see which decision makes audio plugins clearer in the session.

Retention payoff: The payoff is a practical test: Bypass the plugin and name what disappeared.

Visual identity: plugin interface peeled back to reveal math knobs; motif: math knobs; camera: clean lab diagram with waveform, spectrum, and one physical object; motion: signal paths draw in real time while the interface stays simple.

Motion plan: 0-3s plugin interface peeled back to reveal math knobs appears before the full headline, with the math knobs marked in cyan and the problem marked in red; 3-9s show the symptom; 9-18s map DSP systems; 18-24s run the move, Ask what the process changes: level, spectrum, time, or nonlinear shape; 24-30s prove it with Bypass the plugin and name what disappeared and close on Plugin choices get less mystical.
...
Slide 1 thumbnail read: Every plugin math knobs
...
Science anchor: DSP systems

Art and taste anchor: The interface is taste. The process is math.
