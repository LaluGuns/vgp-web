# 091 - Sampling takes photos of air
...
Pattern interrupt: sampling camera frames frozen over a moving waveform appears before the full headline, with the sample frames marked in cyan and the problem marked in red.

Open loop: The viewer waits to see which decision makes sampling audio clearer in the session.

Retention payoff: The payoff is a practical test: Compare pitch-shifted audio at different render settings.

Visual identity: sampling camera frames frozen over a moving waveform; motif: sample frames; camera: clean lab diagram with waveform, spectrum, and one physical object; motion: signal paths draw in real time while the interface stays simple.

Motion plan: 0-3s sampling camera frames frozen over a moving waveform appears before the full headline, with the sample frames marked in cyan and the problem marked in red; 3-9s show the symptom; 9-18s map sampling theorem; 18-24s run the move, Choose sample rate for delivery and processing needs; 24-30s prove it with Compare pitch-shifted audio at different render settings and close on The signal becomes editable.
...
Slide 1 thumbnail read: Sampling takes photos air
...
Science anchor: sampling theorem

Art and taste anchor: The computer is not hearing. It is measuring.
...
Visible text:
- SLIDE 1 | THE HOOK
- [Sampling] takes photos of air
- [Digital audio] stores snapshots of pressure changes.
- www.virzyguns.com
- 100% Art. 100% Science.
...
Headline accent: [continuous wave]
...
Headline accent: [DACs reconstruct]
Support accent: [waveform]
...
Headline accent: [Nyquist Shannon]
...
Headline accent: [Match bandwidth]
...
Headline accent: [Scan for]
Support accent: [Insert a]
...
Headline accent: [clipping]
Support accent: [output ceiling]
