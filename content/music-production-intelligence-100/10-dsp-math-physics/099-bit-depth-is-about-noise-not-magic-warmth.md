# 099 - Bit depth is noise headroom
...
Pattern interrupt: bit depth noise floor under a quiet signal appears before the full headline, with the noise floor marked in cyan and the problem marked in red.

Open loop: The viewer waits to see which decision makes bit depth clearer in the session.

Retention payoff: The payoff is a practical test: Turn down a quiet recording and raise it again to hear noise behavior.

Visual identity: bit depth noise floor under a quiet signal; motif: noise floor; camera: clean lab diagram with waveform, spectrum, and one physical object; motion: signal paths draw in real time while the interface stays simple.

Motion plan: 0-3s bit depth noise floor under a quiet signal appears before the full headline, with the noise floor marked in cyan and the problem marked in red; 3-9s show the symptom; 9-18s map quantization noise; 18-24s run the move, Record and process with enough headroom before export; 24-30s prove it with Turn down a quiet recording and raise it again to hear noise behavior and close on The session stays cleaner.
...
Slide 1 thumbnail read: Bit depth noise headroom
...
Science anchor: quantization noise

Art and taste anchor: Bit depth is margin.
