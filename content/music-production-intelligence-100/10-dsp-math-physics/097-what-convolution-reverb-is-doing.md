# 097 - Convolution reverb copies a space
...
Pattern interrupt: impulse response fingerprint opening a virtual room appears before the full headline, with the impulse room marked in cyan and the problem marked in red.

Open loop: The viewer waits to see which decision makes convolution reverb clearer in the session.

Retention payoff: The payoff is a practical test: Switch IRs with the dry vocal unchanged.

Visual identity: impulse response fingerprint opening a virtual room; motif: impulse room; camera: clean lab diagram with waveform, spectrum, and one physical object; motion: signal paths draw in real time while the interface stays simple.

Motion plan: 0-3s impulse response fingerprint opening a virtual room appears before the full headline, with the impulse room marked in cyan and the problem marked in red; 3-9s show the symptom; 9-18s map linear systems and impulse response; 18-24s run the move, Choose the impulse response for the scene, not the preset name; 24-30s prove it with Switch IRs with the dry vocal unchanged and close on The space feels chosen.
...
Slide 1 thumbnail read: Convolution reverb copies space
...
Science anchor: linear systems and impulse response

Art and taste anchor: A room sample can become a production decision.
