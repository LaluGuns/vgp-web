# 094 - Filters are shape machines
...
Pattern interrupt: filter slope shape machine carving a spectrum appears before the full headline, with the filter shape marked in cyan and the problem marked in red.

Open loop: The viewer waits to see which decision makes audio filters clearer in the session.

Retention payoff: The payoff is a practical test: Compare a steep cut with a gentler one at matched level.

Visual identity: filter slope shape machine carving a spectrum; motif: filter shape; camera: clean lab diagram with waveform, spectrum, and one physical object; motion: signal paths draw in real time while the interface stays simple.

Motion plan: 0-3s filter slope shape machine carving a spectrum appears before the full headline, with the filter shape marked in cyan and the problem marked in red; 3-9s show the symptom; 9-18s map frequency and phase response; 18-24s run the move, Choose gentle filters unless the problem needs surgery; 24-30s prove it with Compare a steep cut with a gentler one at matched level and close on The sound keeps its body.
...
Slide 1 thumbnail read: Filters are shape machines
...
Science anchor: frequency and phase response

Art and taste anchor: A filter has a feel.
