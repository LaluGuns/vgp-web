...
import { mobileNavigate, useSearchParams } from "./shims/next-navigation";

const MOBILE_CALLBACK = "com.virzyguns.flow://auth/callback";
...
export function MobileLoginPage() {
  ...
  useEffect(() => {
    ...
    const handleCallback = async (url: string | undefined | null) => {
      if (!url || !url.startsWith(MOBILE_CALLBACK)) return;
      ...
      mobileNavigate(next, true);
    };

    CapacitorApp.getLaunchUrl()
      .then((launch) => handleCallback(launch?.url))
      .catch(() => {});
  ...
  async function signInWithGoogle() {
    ...
    if (error || !data.url) {
      setStatus("error");
      setMessage(error?.message || t("login.error.authFailed", "Authentication failed. Please try signing in again."));
      return;
    }
  ...
  async function signInWithEmail(event: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: { emailRedirectTo: MOBILE_CALLBACK },
    });
    if (error) {
      setStatus("error");
    ...
    <>
      ...
      <main className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-sm space-y-5">
          <Link href={next} className="inline-flex items-center gap-2 text-xs text-muted-foreground/70 hover:text-white transition-colors">
          ...
          <div className="glass-card p-8 space-y-6">
            ...
            <Button variant="glass" className="w-full h-11" onClick={signInWithGoogle} disabled={!configured || status === "sending"}>
              {status === "sending" ? <Loader2 className="h-4 w-4 animate-apple-loader" /> : <GoogleIcon />}
            ...
            <form onSubmit={signInWithEmail} className="space-y-3">
              ...
              />
              <Button type="submit" className="w-full h-11" disabled={!configured || status === "sending"}>
                {status === "sending" ? <Loader2 className="h-4 w-4 animate-apple-loader" /> : <Mail className="h-4 w-4" />}
                {t("login.magicLink", "Send magic link")}
              </Button>
            </form>

            {message && (
              <p className={`text-xs text-center ${status === "error" ? "text-red-400" : "text-emerald-400"}`}>{message}</p>
            )}
