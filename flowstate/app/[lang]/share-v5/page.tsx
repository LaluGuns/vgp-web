...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const sharePath = localePath(locale, "share-v5");
  const imagePath = localePath(locale, "social-card-v5");
...
export default async function ShareV5Page({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const locale = isSupportedSeoLocale(lang) ? lang : DEFAULT_LOCALE;
