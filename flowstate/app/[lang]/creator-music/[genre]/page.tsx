...
export default async function CreatorGenrePage({ params }: { params: Promise<{ lang: string; genre: string }> }) {
  ...
  return <MarketingShell locale={lang} breadcrumb={[{ name: shared.breadcrumbHome, href: `/${lang}` }, { name: "Creator Music", href: `/${lang}/${CREATOR_MUSIC_PATH}` }, { name: title }]}>
    ...
    <section className="max-w-3xl space-y-5">
      <Link href={`/${lang}/${CREATOR_MUSIC_PATH}`} className="inline-flex items-center gap-1.5 text-xs font-semibold text-white/55 hover:text-[#00e5ff]"><ArrowLeft className="h-3.5 w-3.5" /> All creator music</Link>
    ...
    <CreatorCatalog tracks={tracks} locale={lang} genre={item.genre} title={`${item.genre} creator tracks`} />
    <Link href={`/${lang}/pomodoro-timer-with-music`} className="inline-flex items-center gap-2 text-sm font-semibold text-[#00e5ff] hover:underline underline-offset-4">Pair it with a focus timer <ArrowRight className="h-4 w-4" /></Link>
  </MarketingShell>;
}
