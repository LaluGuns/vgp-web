...
export default async function CreatorMusicPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <MarketingShell locale={lang} breadcrumb={[{ name: shared.breadcrumbHome, href: `/${lang}` }, { name: title }]}>
    ...
    <section className="grid gap-4 rounded-3xl border border-white/[0.08] bg-black/20 p-6 md:grid-cols-[1fr_auto] md:items-center md:p-8">
      ...
      <TrackedLink href={`/${lang}/license`} destination={`/${lang}/license`} source="creator_music_license_cta" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#00e5ff] px-4 py-3 text-xs font-bold uppercase tracking-wider text-black hover:bg-cyan-300">Read the terms <ArrowRight className="h-4 w-4" /></TrackedLink>
    </section>
