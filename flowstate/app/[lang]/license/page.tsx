...
export default async function LicensePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <MarketingShell locale={lang} breadcrumb={[{ name: shared.breadcrumbHome, href: `/${lang}` }, { name: title }]}>
      ...
      <section className="max-w-3xl space-y-5">
        ...
        {localized?.legalReviewNotice && (
          <p className="rounded-xl border border-amber-300/20 bg-amber-300/[0.06] p-3 text-xs leading-relaxed text-amber-100/75">
            {localized.legalReviewNotice}
          </p>
        )}
      ...
      <section className="grid gap-8 md:grid-cols-2">
        <div>
          <h2 className="text-xl font-bold text-white">{copy.allowedHeading}</h2>
          <ul className="mt-4 space-y-3">
            {copy.allowed.map((item) => <li key={item} className="flex gap-2.5 text-sm leading-relaxed text-white/70"><Check className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />{item}</li>)}
          </ul>
        </div>
        <div>
          <h2 className="text-xl font-bold text-white">{copy.notAllowedHeading}</h2>
          <ul className="mt-4 space-y-3">
            {copy.notAllowed.map((item) => <li key={item} className="flex gap-2.5 text-sm leading-relaxed text-white/70"><X className="mt-0.5 h-4 w-4 shrink-0 text-rose-400" />{item}</li>)}
          </ul>
        </div>
      </section>
      ...
      <section className="max-w-3xl space-y-4">
        <h2 className="text-xl font-bold text-white">{shared.faqHeading}</h2>
        {faq.map((item) => <div key={item.q} className="border-b border-white/[0.08] pb-4"><h3 className="font-semibold text-white">{item.q}</h3><p className="mt-1 text-sm leading-relaxed text-white/60">{item.a}</p></div>)}
      </section>
    </MarketingShell>
  );
}
