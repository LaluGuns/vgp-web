...
const DESCRIPTION = "Deep work music + Pomodoro timer with an original soundtrack, produced in-house by Virzy Guns.";

export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  const { lang } = await params;
  const locale = isSupportedSeoLocale(lang) ? lang : DEFAULT_LOCALE;
  const sharePath = localePath(locale, "share-v6");
  const imagePath = localePath(locale, "social-card-v6");
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      url: `${SITE}${sharePath}`,
      images: [{ url: `${SITE}${imagePath}`, width: 1200, height: 630, alt: TITLE }],
    },
    twitter: {
...
export default async function ShareV6Page({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const locale = isSupportedSeoLocale(lang) ? lang : DEFAULT_LOCALE;
  const destination = localePath(locale);

  return (
    <main className="min-h-screen bg-[#060216] text-white flex items-center justify-center px-6">
      <div className="text-center">
        <p className="text-sm text-white/50">Opening Flow...</p>
        <a className="mt-3 inline-block text-cyan-300 underline underline-offset-4" href={destination}>Continue to Flow</a>
      </div>
