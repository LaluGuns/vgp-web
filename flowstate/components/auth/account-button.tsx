...
import { User as UserIcon, LogOut, Sparkles, ChevronDown, BarChart2, Trash2 } from "lucide-react";
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        <span className="w-5 h-5 rounded-full bg-primary/10 border border-primary/25 text-primary text-[10px] font-bold flex items-center justify-center overflow-hidden uppercase shrink-0">
          {avatarUrl ? (
            <img src={avatarUrl} alt={displayName || "User"} className="w-full h-full object-cover relative z-10" />
          ) : (
      ...
      {open && (
        <div className="absolute right-0 top-11 w-56 glass-card p-2 !rounded-2xl z-50 animate-in fade-in-0 zoom-in-95 backdrop-blur-xl border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.5),inset_0_1px_rgba(255,255,255,0.15)]">
          <div className="px-3 py-2 border-b border-white/[0.08] mb-1">
            <p className="text-xs font-semibold text-white truncate">{displayName || t("dashboard.signedIn", "Signed in")}</p>
          ...
          <Link href="/delete-account" onClick={() => setOpen(false)}>
            <button className="w-full text-left px-3 py-2.5 rounded-xl text-xs text-white/45 hover:text-rose-300 hover:bg-rose-500/10 flex items-center gap-2.5 transition-all duration-200 mt-1">
          ...
          <form onSubmit={handleSignOut}>
            <button
              type="submit"
              className="w-full text-left px-3 py-2.5 rounded-xl text-xs text-white/50 hover:text-white hover:bg-white/[0.06] flex items-center gap-2.5 transition-all duration-200 border-t border-white/[0.08] mt-1 pt-2.5"
            >
